...
class LTFMPredictor:
    ...
    def load_models(self, graph2vec_path: str, ltfm_checkpoint_path: str, partition_info_path: str) -> bool:
        ...
        try:
            ...
            # Load LTFM model (use number of partitions obtained from partition information)
            self.ltfm_model = LTFMModel(
                ...
                num_regions=self.n_clusters,  # Use loaded number of partitions
            ...
            logger.info(f"Model loading complete (partitions: {self.n_clusters})")
            return True
    ...
    def get_partitions_from_saved_info(self) -> Tuple[List[List[str]], List[nx.Graph]]:
        """
        Build partitions and subgraphs based on saved partition information

        Returns:
            Tuple[List[List[str]], List[nx.Graph]]: (List of partition nodes, List of partition subgraphs)
        """
        try:
            if self.partition_labels is None or self.partition_node_names is None:
                logger.error("Partition information is not loaded")
                return [], []
    ...
    def predict_anomaly(self, current_pressure: pd.DataFrame) -> Dict:
        ...
        Args:
            current_pressure: Current pressure data [time, nodes]
        ...
        try:
            ...
            if self.normal_pressure is None:
                logger.error("Normal pressure baseline is not set")
                return {}

            # Calculate pressure difference (normal pressure - current pressure)
            pressure_diff = self.normal_pressure - current_pressure

            # Calculate node weights (mean of pressure differences)
            node_weights = {}
            ...
            # Use partition information saved during training (no re-partitioning)
