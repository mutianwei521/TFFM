...
"""
Training Module
Implements the model training process, including loss function calculation and backpropagation
"""
...
def custom_collate_fn(batch):
    """
    Custom collate function for handling batch data containing NetworkX graphs

    Args:
        batch: List of batch data

    Returns:
        List: Original batch data (without collate operation)
    """
    return batch


class PrecomputedDataset(Dataset):
    """Precomputed embedding dataset: directly store tensors to avoid repeated encoding"""
...
class WaterNetworkDataset(Dataset):
    """Water Distribution Network Dataset (compatible with old interface)"""
    ...
    def __init__(self, scenarios: List[Dict]):
        ...
        Args:
            scenarios: List of scenarios, each containing:
                ...
                - global_label: Global label (0 for normal, 1 for abnormal)
                - region_label: Region label (0 for normal, >0 for anomalous region index)
        """
...
class LTFMTrainer:
    ...
    def initialize_models(self):
        ...
        try:
            ...
            # Optimizer - Using AdamW with improved parameters
            ...
            # Learning rate scheduler - Using ReduceLROnPlateau (more suitable for our task)
    ...
    def generate_training_scenarios(self, epanet_handler: EPANETHandler,
        ...
        try:
            ...
            # Get network graph and partition information
            ...
            # Generate normal scenarios - increasing diversity
            ...
            for i in tqdm(range(n_normal), desc="Generating normal scenarios"):
                ...
                for node in node_names:
                    # Add small random variations to simulate natural fluctuations during normal operation
                    # Variation range: -0.05 to +0.05 (very small compared to abnormal scenarios)
                ...
                for subgraph in subgraphs:
                    ...
                    for node in node_names:
                        if node in subgraph.nodes():
                            # Small correlated variations for nodes within the region
                ...
                scenario = {
                    ...
                    'anomaly_node_idx': -1,  # No leak node in normal scenario
                    'anomaly_node_name': None,
                    'node_names': node_names
                }
                scenarios.append(scenario)
            
            # Generate abnormal scenarios - enhanced version
            n_anomaly = n_scenarios - n_normal
            anomaly_count = 0
            max_attempts = n_anomaly * 3  # Maximum attempts up to 3 times

            for attempt in tqdm(range(max_attempts), desc="Generating abnormal scenarios"):
                ...
                # Get sensitivity features of anomaly node
                sensitivity_features = sensitivity_analyzer.get_sensitivity_features(anomaly_node_idx)

                if not sensitivity_features:
                    logger.debug(f"Node {anomaly_node} sensitivity features are empty, skipping")
                    continue

                # Build node weights (pressure differences)
                ...
                if len(pressure_sensitivity) != n_nodes:
                    logger.debug(f"Node {anomaly_node} pressure sensitivity length mismatch, skipping")
                    continue

                # Data augmentation: add noise and transformations
                ...
                # Determine anomalous region - fix label encoding
                partition_labels = partition_info['partition_labels']
                # Anomalous region label: 0=normal, 1=region 0 abnormal, 2=region 1 abnormal, 3=region 2 abnormal...
                anomaly_region = partition_labels[anomaly_node_idx] + 1  # +1 because 0 indicates normal

                # Build region weights - enhance features of anomalous regions
                region_weights = []
                for region_id, subgraph in enumerate(subgraphs):
                    ...
                    for node in subgraph.nodes():
                        ...
                        # If it's an anomalous region, significantly enhance weight
                        if partition_labels[node_idx] == (anomaly_region - 1):
                            weight *= np.random.uniform(3.0, 5.0)  # Greatly enhance anomalous region weight
                        else:
                            # Non-anomalous region, appropriately reduce weight
                ...
                scenario = {
                    ...
                    'region_weights': region_weights,
                    'global_label': 1,  # Abnormal
                    'region_label': anomaly_region,  # Abnormal region index
                ...
                variants_to_generate = min(2, n_anomaly - anomaly_count)  # Maximum 2 variants per base scenario
            ...
            logger.info(f"Successfully generated {anomaly_count} abnormal scenarios (Target: {n_anomaly})")
            
            logger.info(f"Training scenario generation complete: {len(scenarios)} scenarios")
            return scenarios

        except Exception as e:
            logger.error(f"Training scenario generation failed: {e}")
            return []

    def _augment_pressure_sensitivity(self, pressure_sensitivity: np.ndarray, anomaly_node_idx: int) -> np.ndarray:
        """
        Perform data augmentation on pressure sensitivity
        ...
        # 2. Enhance the impact of anomaly nodes
    ...
    def _create_scenario_variant(self, base_scenario: Dict, node_names: List[str],
        ...
        Returns:
            Optional[Dict]: Variant scenario or None
        """
        try:
            variant = base_scenario.copy()
            # Retain node-level info like anomaly_node_idx
            ...
            # Variant 3: Multi-node anomalies (multiple nodes within the same region)
                ...
                if same_region_nodes:
                    # Randomly select 1-2 extra nodes
            ...
            # Variant 4: Temporal change simulation (via weight gradient changes)
            elif variant_type == 'temporal':
                temporal_global_weights = {}
                for node, weight in base_scenario['global_weights'].items():
                    # Simulate temporal changes: add periodic changes
            ...
            logger.error(f"Failed to generate training scenario: {e}")
            return []
    ...
    def precompute_embeddings(self, scenarios: List[Dict]) -> Tuple[
        ...
        """
        Precompute embeddings for all scenarios, encode once for all
        
        Args:
            scenarios: List of scenarios
            
        Returns:
            Tuple: (Global embedding, List of region embeddings, Global label, Region label)
        """
        ...
        logger.info(f"Precompution complete: Global embeddings {global_emb_tensor.shape}, "
                    f"{num_regions} Region embeddings {region_emb_tensors[0].shape}")
        
        # Verify diversity in embeddings
        emb_std = global_emb_tensor.std(dim=0).mean().item()
        logger.info(f"Global embedding std (averaged across samples): {emb_std:.6f}")
        if emb_std < 1e-6:
            logger.warning("⚠️ Embeddings have almost no variance! Encoder might not correctly capture feature differences")
    ...
    def prepare_batch_data(self, batch: List[Dict]) -> Tuple[torch.Tensor, List[torch.Tensor], 
        ...
        """
        Prepare batch data (compatible with old interface, but using precompute_embeddings is recommended)
    ...
    def train_epoch(self, dataloader: DataLoader) -> Tuple[float, float]:
        """
        Train one epoch (supports precomputed and old modes)
        ...
        for batch in tqdm(dataloader, desc=f"Training Epoch {self.current_epoch}"):
            # Check if it's precomputed mode or old mode
    ...
    def validate_epoch(self, dataloader: DataLoader) -> Tuple[float, float, Dict]:
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Validating"):
                # Check for precomputed mode vs old mode
                if isinstance(batch, (tuple, list)) and len(batch) == 4 and isinstance(batch[0], torch.Tensor):
                    global_embs, region_embs, global_labels, region_labels = batch
                    global_embs = global_embs.to(self.device)
                    region_embs = [r.to(self.device) for r in region_embs]
        ...
        if len(all_global_labels) > 0:
            ...
            logger.info(f"  Global label distribution: Normal={all_global_labels.count(0)}, Abnormal={all_global_labels.count(1)}")
            global_preds_int = [int(p) for p in all_global_preds]
            logger.info(f"  Global prediction distribution: Normal={global_preds_int.count(0)}, Abnormal={global_preds_int.count(1)}")
            ...
            logger.info(f"  Unique region labels: {set(all_region_labels)}")
            logger.info(f"  Unique region predictions: {set(all_region_preds)}")

            if len(set(all_global_preds)) == 1:
                logger.warning("⚠️ All global predictions are the same! The model might not have learned useful features.")

            if len(set(all_region_preds)) == 1:
                logger.warning("⚠️ All region predictions are the same! The model might not have learned useful features.")
    ...
    def calculate_metrics(self, global_preds: List, global_labels: List,
        ...
        try:
            ...
            if len(unique_labels) > 1:
                ...
                logger.debug(f"AUC set to 0.0 because there is only one type of label: {unique_labels}")

            # Region localization metrics
            region_preds = np.array(region_preds)
            region_labels = np.array(region_labels)

            metrics['region_acc'] = accuracy_score(region_labels, region_preds)

            # Only consider regional localization accuracy for abnormal samples
    ...
    def train(self, train_scenarios: List[Dict], val_scenarios: List[Dict], skip_stage1: bool = False) -> bool:
        """
        Train model (use precomputed embeddings to accelerate)
        ...
        """
        try:
            # Precompute all embeddings (done once, avoid re-encoding every epoch)
            logger.info("=" * 60)
            logger.info("Step 1: Precompute embeddings (executed only once)")
            logger.info("=" * 60)
            ...
            if skip_stage1:
                ...
                if os.path.exists(checkpoint_path):
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to load model: {e}. Will continue training...")
                else:
                    logger.warning(f"Checkpoint not found {checkpoint_path}. Will continue training...")
            ...
            logger.info(f"Training set label distribution - Normal: {train_global_labels.count(0)}, Abnormal: {train_global_labels.count(1)}")
            logger.info(f"Validation set label distribution - Normal: {val_global_labels.count(0)}, Abnormal: {val_global_labels.count(1)}")
            ...
            for epoch in range(self.config['training']['epochs']):
                ...
                # Improved early stopping check - comprehensively consider multiple metrics
                # Calculate composite score: loss + regional accuracy
                region_acc = val_metrics.get('anomaly_region_acc', 0.0)
                global_auc = val_metrics.get('global_auc', 0.0)

                # Composite score: higher is better (lower loss + higher accuracy)
                ...
                if region_acc > 0.1:  # If regional accuracy improves, increase patience
                    patience = patience + 5

                if self.patience_counter >= patience:
                    logger.info(f"Early stopping triggered, stopped training at epoch {self.current_epoch}")
            ...
            logger.info("LTFM training complete")
            
            # Save precomputed embeddings for NodeLocalizer usage
            self._precomputed_train = (train_global, train_regions, train_glabels, train_rlabels)
            self._precomputed_val = (val_global, val_regions, val_glabels, val_rlabels)
    ...
    def train_node_localizer(self, train_scenarios: List[Dict], 
        ...
        Use frozen LTFM intermediate features + sensitivity vector -> predict leaky node
        Includes data augmentation (40x Gaussian noise) to solve sample insufficiency
        
        Args:
            train_scenarios: Training scenarios (contains anomaly_node_idx)
            val_scenarios: Validation scenarios
        ...
        try:
            logger.info("=" * 60)
            logger.info("Stage 2: Train NodeLocalizer (Node-level leak localization)")
            logger.info("=" * 60)
            
            # 1. Filter anomaly scenarios (only anomaly scenarios have leak node labels)
            ...
            if not train_anomaly or not val_anomaly:
                logger.error("No anomalous scenario for NodeLocalizer training")
                return False
            ...
            if n_nodes == 0:
                logger.error("Failed to get node names info")
                return False
            
            logger.info(f"Number of network nodes: {n_nodes}")
            
            # 3. Use precomputed embeddings for intermediate features extraction (fast mode)
            logger.info("Using precomputed embeddings to extract region features (skipping graph encoding)...")
            self.ltfm_model.eval()
            ...
            if hasattr(self, '_precomputed_train') and hasattr(self, '_precomputed_val'):
                ...
                logger.info("⚠️ No cached embeddings, recalculating...")
                train_global, train_regions, train_glabels, train_rlabels = \
                    self.precompute_embeddings(train_scenarios)
                val_global, val_regions, val_glabels, val_rlabels = \
                    self.precompute_embeddings(val_scenarios)
            
            # Filter anomaly scenarios indices
            ...
            # Statistics on regional constraints
            avg_region_size = train_masks.float().sum(dim=1).mean().item()
            logger.info(f"Raw training features: region_feat={train_features.shape}, "
                        f"sens={train_sens.shape}, labels={train_labels.shape}")
            logger.info(f"Regional constraints: On average, each sample is classified among only {avg_region_size:.0f}/{n_nodes} nodes")
            
            # 4. Data augmentation: Add Gaussian noise to create 40x more training samples
            augment_factor = 40
            logger.info(f"Data augmentation: {augment_factor}x Gaussian noise expansion...")
            ...
            for aug_i in range(augment_factor):
                # Increasing noise strength: 0.05 ~ 0.5
                ...
                # Randomly zero out 15% of sensitivity values -> cancelled
                # mask_prob = 0.15
                ...
                aug_masks_list.append(train_masks.clone())  # Region masks unchanged
            ...
            # 6. Create DataLoader (contains region mask)
            ...
            for epoch in range(epochs):
                ...
                for feat_batch, sens_batch, label_batch, mask_batch in train_loader:
                    ...
                    # Region constraint: set logits of nodes outside region to -inf
                ...
                if patience_counter >= 25:
                    logger.info(f"NodeLocalizer early stopping triggered, stopped at epoch {epoch+1}")
                    break
            
            logger.info(f"NodeLocalizer training complete - Best Node Acc (±1 hop): {best_node_acc_hop:.4f}")
            return True
    ...
    def _extract_node_features_fast(self, scenarios: List[Dict], 
        ...
        Args:
            ...
            region_embs_list: Precomputed list of region embeddings
            region_labels: Region labels [N]
            node_names: Node names list
            
        Returns:
            Tuple: (region_features, sensitivity_vectors, node_labels, region_masks)
                   region_masks: [n_samples, n_nodes] bool, True=node is in abnormal region
        """
        try:
            n_nodes = len(node_names)
            batch_size = 128
            
            # 1. Extract embeddings of anomalous scenarios
            ...
            # 2. Extract intermediate features via batch LTFM forward propagation
            all_region_feats = []
            ...
            with torch.no_grad():
                for start in range(0, n_anomaly, batch_size):
                    ...
                    # Take features of the anomalous region for each sample
            ...
            # 3. Construct sensitivity vector + node label + region mask
            sensitivity_vectors = []
            node_labels = []
            region_masks = []
            ...
            for idx in anomaly_indices:
                ...
                # Build region mask: only allow nodes inside the anomalous region
                ...
                if pl is not None and rlabel > 0:
                    ...
                    # fallback: no constraints (all nodes selectable)
            ...
            logger.error(f"Failed during fast feature extraction: {e}")
            import traceback
            traceback.print_exc()
            return None, None, None, None
    ...
    def calculate_node_accuracy(self, preds: List[int], labels: List[int],
        ...
        """
        Calculate node level accuracy (exact + ±1 hop tolerance)
        
        Args:
            preds: List of predicted node indices
            labels: List of true node indices  
            node_names: List of node names
            network_graph: Network graph (to lookup neighbors)
        ...
        for pred_idx, true_idx in zip(preds, labels):
            ...
            # Neighbor tolerance: check if predicted node is ±1 hop neighbor of the true node
            try:
