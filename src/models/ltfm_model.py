...
Implements the complete architecture of Adaptor layer, LTFM layer, and VT SELECTOR layer
...
class AdaptorLayer(nn.Module):
    """Enhanced adaptor layer that adapts Graph2Vec embeddings to LTFM input format"""

    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, dropout: float = 0.1):
        """
        Initialize adaptor layer

        Args:
            input_dim: Input dimension (Graph2Vec embedding dimension)
        ...
        # Residual connection projection layer (when input/output dimensions differ)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
        ...
        """
        Forward pass (supports cross-attention, query and key/value can have different seq_len)
...
class LTFMLayer(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int, hidden_dim: int, dropout: float = 0.1):
        ...
        # Feature matching - using custom MultiHeadAttention to avoid PyTorch 2.7 compatibility issues
...
class VTSelectorLayer(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_regions: int, dropout: float = 0.1, num_heads: int = 8):
        ...
        # Global feature processing - enhanced (using LayerNorm instead of BatchNorm to support batch_size=1)
        ...
        # Region feature processing - enhanced (using LayerNorm instead of BatchNorm to support batch_size=1)
        ...
        # Multi-head attention mechanism - using custom MultiHeadAttention to avoid PyTorch 2.7 compatibility issues
        ...
        # Cross-attention - using custom MultiHeadAttention
        ...
        # Simplified classification head - global (using LayerNorm instead of BatchNorm to support batch_size=1)
        ...
        # Simplified classification head - region (shared weights with region-specific biases)
        ...
        # Output scaling parameters - make model output more discriminative
        self.global_output_scale = nn.Parameter(torch.tensor(2.0))
        self.region_output_scale = nn.Parameter(torch.tensor(3.0))  # Larger scaling for region output

        self.layer_norm = nn.LayerNorm(embed_dim)
        self.dropout = nn.Dropout(dropout)

        # Input dimension adapter (handles dimension changes from enhanced features)
        self.input_adapter = nn.Linear(embed_dim, embed_dim)  # Dynamically adjusted at runtime
    ...
    def _init_weights(self):
        ...
        # Special initialization for classification layers - larger initial variance for output
    ...
    def forward(self, global_features: torch.Tensor,
        ...
        Returns:
            Tuple[torch.Tensor, List[torch.Tensor]]: (global scores, region scores list)
        """
        ...
        # 3. Build sequence: [global features, region features 1, region features 2, ...]
        if processed_regions:
            ...
            # 4. Self-attention mechanism - enable feature interaction
            ...
            # 5. Cross-attention - global features as query, region features as key and value
            ...
            if region_kv.shape[1] > 0:
                ...
                enhanced_global = torch.cat([
                    attended_features[:, 0, :],  # Original global features
                    cross_attended.squeeze(1)    # Cross-attention features
                ], dim=-1)
            ...
            for i in range(len(processed_regions)):
                if i < attended_features.shape[1] - 1:
                    # Combine original region features and attention-enhanced features
        ...
        # 8. Region classification - shared classifier + region-specific layers
        region_scores = []
...
class LTFMModel(nn.Module):
    ...
    def forward(self, global_graph_embedding: torch.Tensor,
        ...
        Args:
            ...
            return_features: Whether to return intermediate features (for NodeLocalizer)

        Returns:
            If return_features=False: (global anomaly scores, region anomaly scores list)
            If return_features=True: (global anomaly scores, region anomaly scores list, region features list)
        """
        ...
        for ltfm_layer in self.ltfm_layers:
            # Process each region feature with global features through LTFM
            processed_regions = []
            for region_feat_seq in region_features_seq:
                processed_region = ltfm_layer(global_features, region_feat_seq)
                processed_regions.append(processed_region)

            # Update global features (using average of all region features)
            if processed_regions:
    ...
    def compute_loss(self, global_score: torch.Tensor, region_scores: List[torch.Tensor],
        ...
        Args:
            global_score: Global anomaly scores [batch_size, 1]
        ...
        """
        # 1. Global anomaly detection loss - using Focal Loss for binary classification
        ...
        # 2. Region anomaly localization loss - using weighted Focal Loss
        if region_scores:
            # Stack region scores
            region_scores_tensor = torch.cat(region_scores, dim=1)  # [batch_size, num_regions]

            # Add normal class (index 0) scores
            ...
            region_focal_loss = self.focal_loss(all_scores, region_labels.long(),
                                               alpha=2.0, gamma=2.0)  # Increased weight for region detection

            # 3. Add region consistency loss - encourage anomalous regions to have higher scores
            consistency_loss = 0.0
            if len(region_scores) > 1:
                # Compute variance of region scores for anomalous samples, encouraging anomalous regions to stand out
                ...
                if anomaly_mask.sum() > 0:
                    ...
                    if anomaly_region_scores.shape[0] > 0:
                        # Encourage diversity in anomalous region scores (using positive loss)
                        region_std = torch.std(anomaly_region_scores, dim=1)
                        # Use 1/(std+epsilon) to encourage larger variance, avoiding negatives
                        epsilon = 1e-6
        ...
        if len(region_scores) > 1 and len(region_scores_tensor) > 0:
            # For anomalous samples, force anomalous region scores much higher than other regions
            anomaly_mask = global_labels > 0
            if anomaly_mask.sum() > 0:
                anomaly_region_scores = region_scores_tensor[anomaly_mask]
                ...
                if anomaly_region_scores.shape[0] > 0:
                    # For each anomalous sample, its corresponding region score should be higher than others
                    for i, (scores, true_region) in enumerate(zip(anomaly_region_scores, anomaly_region_labels)):
                        if 0 <= true_region < len(scores):
                            # True anomalous region score
    ...
    def predict(self, global_graph_embedding: torch.Tensor,
        ...
        with torch.no_grad():
            ...
            if region_scores:
                ...
                # Use temperature scaling for smoother predictions
                ...
                # Predict: select the class with highest probability
                region_pred = torch.argmax(probs, dim=1)

                # Add confidence threshold: predict as normal if max probability is too low
                max_probs = torch.max(probs, dim=1)[0]
                confidence_threshold = 0.35  # Confidence threshold
                low_confidence_mask = max_probs < confidence_threshold
                region_pred[low_confidence_mask] = 0  # Predict as normal for low confidence
