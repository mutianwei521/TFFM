...
class Graph2VecEncoder(nn.Module):
    """Graph2Vec encoder that converts graph structures to embedding vectors (optimized version)"""
    ...
    def __init__(self, embedding_dim: int = 128, wl_iterations: int = 3,
        ...
        Args:
            ...
            wl_iterations: Weisfeiler-Lehman iteration count (kept for interface compatibility)
            workers: Number of parallel workers (kept for interface compatibility)
            epochs: Training epochs (kept for interface compatibility)
            min_count: Minimum word frequency (kept for interface compatibility)
            learning_rate: Learning rate (kept for interface compatibility)
        """
        ...
        self.model = None  # Kept for compatibility, no longer uses Word2Vec
        self.vocabulary = set()
        self.graph_embeddings = {}
        
        # Topology feature cache: compute only once for the same graph structure
        self._topology_cache = {}
        
        # Learnable feature projection layer: projects raw features to embedding_dim
        self._raw_feature_dim = None  # Determined at first encoding
        self._projection = None  # Lazy initialization
        
        # Feature normalization statistics (computed during train_model)
    ...
    def _get_graph_hash(self, graph: nx.Graph) -> str:
        """
        Compute topology hash of the graph (excluding weights), used for caching topology features
        """
    ...
    def _extract_topology_features(self, graph: nx.Graph) -> np.ndarray:
        """
        Extract topology features of the graph (weight-independent, cacheable)
        ...
        # Basic graph statistics (5 dimensions)
        ...
        # Clustering coefficient (1 dimension)
        features.append(nx.average_clustering(graph))
        
        # Degree distribution features (6 dimensions)
        ...
        # Centrality features (4 dimensions)
        try:
        ...
        # Triangle features (2 dimensions)
        try:
    ...
    def _extract_weight_features(self, graph: nx.Graph, 
        ...
        # 2. Quantile features (5 dimensions)
        for q in [10, 25, 75, 90, 95]:
            features.append(np.percentile(weights, q))
        
        # 3. Weight distribution histogram (15 dimensions)
        ...
        # 4. Positive/negative/zero statistics (3 dimensions)
        ...
        # 5. Outlier features (3 dimensions)
        if np.std(weights) > 1e-10:
            z_scores = np.abs((weights - np.mean(weights)) / np.std(weights))
            features.append(np.sum(z_scores > 2.0) / n_total)  # Proportion outside 2-sigma
            features.append(np.sum(z_scores > 3.0) / n_total)  # Proportion outside 3-sigma
            features.append(np.max(z_scores))                   # Maximum Z-score
        else:
            features.extend([0.0, 0.0, 0.0])
        
        # 6. Topology-weight interaction features (10 dimensions)
        ...
        if len(degrees) > 0 and len(weights) > 0:
            ...
            if neighbor_weight_diffs:
                ...
                features.append(np.std(nwd))    # Std of neighbor weight differences
        ...
        # Ensure fixed length (6 dimensions padding/truncation to reach 50)
    ...
    def train_model(self, graphs: List[nx.Graph],
        ...
        """
        Train Graph2Vec model (compute feature normalization statistics and initialize projection layer)
        ...
        """
        try:
            logger.info(f"Starting Graph2Vec encoder training (numerical feature mode)")
            
            # Pre-compute topology features for all graphs (cached)
            for graph in graphs:
            ...
            logger.info(f"Graph2Vec encoder training complete: raw feature dim={self._raw_feature_dim}, "
                        f"output dim={self.embedding_dim}")
            
            # If set to freeze parameters, disallow further training
