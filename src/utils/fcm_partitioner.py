...
class FCMPartitioner:
    ...
    def __init__(self, n_clusters: int = 5, m: float = 2.0, 
        ...
        Args:
            n_clusters: Number of partitions
            m: Fuzziness exponent
            max_iter: Maximum number of iterations
            error: Convergence error threshold
        """
    ...
    def prepare_features(self, adjacency_matrix: np.ndarray,
        ...
        Args:
            ...
            node_weights: Node weights (average pressure sensitivity values)
            pipe_lengths: Pipe length dictionary
        ...
        try:
            ...
            logger.debug(f"  adjacency_matrix shape: {adjacency_matrix.shape}, dtype: {adjacency_matrix.dtype}")
            logger.debug(f"  node_weights shape: {node_weights.shape}, dtype: {node_weights.dtype}")
            ...
            if not np.issubdtype(node_weights.dtype, np.number):
                logger.error(f"node_weights contains non-numeric data, dtype: {node_weights.dtype}")
                return np.array([])
            ...
            max_node_weights = np.max(node_weights, axis=1)   # Maximum sensitivity
            ...
            # 4. Pipe length features (average length of pipes connected to each node)
            ...
            for pipe_name in pipe_names:
                # Need to get pipe connection info from EPANET handler
            ...
            feature_matrix = np.column_stack([
                ...
                max_node_weights,       # Maximum pressure sensitivity
            ...
            logger.info(f"Feature matrix prepared: {feature_matrix.shape}")
    ...
    def partition_network(self, feature_matrix: np.ndarray) -> bool:
        """
        Partition the network using FCM
        ...
        try:
            ...
            if not np.issubdtype(feature_matrix.dtype, np.number):
                logger.error(f"Feature matrix contains non-numeric data, dtype: {feature_matrix.dtype}")
                return False
            ...
            # Check if partition count is reasonable
            n_samples = feature_matrix.shape[0]
            if self.n_clusters >= n_samples:
                logger.error(f"Number of partitions {self.n_clusters} >= number of samples {n_samples}")
                return False

            logger.debug(f"Starting FCM clustering: samples={n_samples}, features={feature_matrix.shape[1]}, clusters={self.n_clusters}")

            # Transpose feature matrix to match skfuzzy input format
            ...
            # Get partition label for each node (cluster with highest membership)
            ...
            logger.info(f"FCM partitioning complete: {self.n_clusters} partitions, silhouette score: {silhouette_avg:.3f}")
            
            # Output partition statistics
            for i in range(self.n_clusters):
                cluster_size = np.sum(self.partition_labels == i)
                logger.info(f"Partition {i}: {cluster_size} nodes")
    ...
    def get_partition_subgraphs(self, graph: nx.Graph, 
        ...
        try:
            if self.partition_labels is None:
                logger.error("No partition results found")
                return []
            
            subgraphs = []
            
            for cluster_id in range(self.n_clusters):
                # Get nodes in current partition
    ...
    def optimize_partition_number(self, feature_matrix: np.ndarray,
        ...
        Args:
            feature_matrix: Feature matrix
            min_clusters: Minimum number of partitions
            max_clusters: Maximum number of partitions
        ...
        try:
            ...
            logger.info(f"Optimizing partition count: {min_clusters}-{max_clusters}")
            
            for n in range(min_clusters, max_clusters + 1):
                # Temporarily set partition count
                ...
                # Restore original partition count
                self.n_clusters = original_n_clusters
            
            # Set optimal partition count
            self.n_clusters = best_n_clusters
            logger.info(f"Optimal partition count: {best_n_clusters}, silhouette score: {best_score:.3f}")
            
            return best_n_clusters
            
        except Exception as e:
            logger.error(f"Failed to optimize partition count: {e}")
    ...
    def get_partition_info(self) -> Dict:
        ...
        Returns:
            Dict: Partition information dictionary
        """
        if self.partition_labels is None:
            return {}
    ...
    def check_connectivity(self, node_connections: np.ndarray, cluster_nodes: np.ndarray) -> np.ndarray:
        """Check node connectivity using Warshall's algorithm"""
        n = len(cluster_nodes)
    ...
    def remove_outliers_iteratively(self, wn, nodes: List[str], demands: List[str],
        ...
        Args:
            ...
            seed: Random seed
            output_dir: Output directory (for saving visualization images)
        ...
        while number_iter < max_iterations:
            ...
            # Handle type 1 outliers: based on neighbor node label consistency
            for i, node in enumerate(nodes):
                if all_labels[i] != 99999:  # Exclude special markers
                    ...
                    if len(connected_nodes) > 0:
                        ...
                        # If 0 is among max-count labels and there are other labels, remove 0
            ...
            for cluster in range(1, int(np.max(all_labels)) + 1):
                ...
                # Compute KNN distance for each node
                knn_distances = []
                for i in range(len(cluster_nodes)):
                ...
                # Compute statistics and mark outliers
        ...
        # Check if any partition was completely eliminated, if so restore largest connected component
        ...
        # Final partition count verification
        ...
        if unassigned_count > 0:
            ...
            if final_unassigned == 0:
                logger.info("✅ All nodes successfully assigned to partitions via nearest neighbor")
            else:
                logger.info(f"⚠️ {final_unassigned} nodes still unassigned after nearest neighbor assignment")

            logger.info(f"Outlier detection and nearest neighbor assignment complete, iterations: {number_iter}")
            ...
            logger.info(f"Outlier detection complete, iterations: {number_iter}")
    ...
    def assign_unassigned_nodes_by_nearest_neighbor(self, wn, nodes: List[str], demands: List[str],
        ...
        logger.info(f"Starting nearest neighbor partition assignment for {len(unassigned_indices)} unassigned demand nodes")
        ...
        for unassigned_idx in unassigned_indices:
            ...
            # Iterate over all partitions to find nearest node
    ...
    def visualize_partition(self, wn, nodes: List[str], labels: np.ndarray,
        ...
        """
        Visualize partition results (using WNTR plotting with original coordinates, Nature journal style)
        ...
        try:
            ...
            # Assign high-contrast colors for each partition (Nature journal color palette)
            ...
            for i, label in enumerate(unique_labels):
                if label == 0:
                    continue  # Skip unassigned label
                color_idx = (int(label) - 1) % len(distinct_colors)
                color_map[label] = distinct_colors[color_idx]

            # Plot network using WNTR (pipes only)
            wntr.graphics.plot_network(
                wn,
                node_size=0,  # Don't plot nodes
                link_width=1.5,
                add_colorbar=False,
                ax=ax
            )
            ...
            # Plot assigned nodes (without node labels)
            ...
            # Add legend (enlarged, avoid occlusion)
            legend = ax.legend(
                loc='center left',
                bbox_to_anchor=(1.05, 0.5),  # Place to the right of the figure
                fontsize=12,
                frameon=True,
                ...
                # title='Partitions',
                # title_fontsize=13,
                markerscale=0.5,  # Enlarge legend markers to avoid occlusion
            ...
            logger.error(f"Failed to visualize partition: {e}")
            import traceback
            traceback.print_exc()
            return False
