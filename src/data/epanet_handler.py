...
Handles network import, hydraulic simulation, and pressure data retrieval using the WNTR library
"""
...
class EPANETHandler:
    ...
    def load_network(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Network loaded successfully: {len(self.node_names)} nodes, {len(self.pipe_names)} pipes")
            return True
    ...
    def run_hydraulic_simulation(self, duration_hours: int = 24, 
        ...
        try:
            ...
            logger.info(f"Hydraulic simulation complete: {duration_hours}-hour simulation")
            return True
    ...
    def get_pressure_data(self) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: Pressure data, rows are time steps, columns are nodes
        """
        if self.results is None:
            logger.error("No simulation results found, please run hydraulic simulation first")
            return pd.DataFrame()
    ...
    def get_flow_data(self) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: Flow data, rows are time steps, columns are pipes
        """
        if self.results is None:
            logger.error("No simulation results found, please run hydraulic simulation first")
            return pd.DataFrame()
    ...
    def get_total_network_demand(self) -> float:
        """
        Calculate total demand of the entire network
    ...
    def modify_node_demand(self, node_name: str, demand_ratio: float = 0.2) -> bool:
        ...
        try:
            ...
            if abs(original_demand) < 1e-6:  # Use small threshold for floating point comparison
                total_demand = self.get_total_network_demand()
                new_demand = total_demand * 0.03  # 3%
                logger.info(f"Node {node_name} has zero original demand, using 3% of total network demand: {new_demand:.3f}")
            else:
    ...
    def get_pipe_lengths(self) -> Dict[str, float]:
        ...
        Returns:
            Dict[str, float]: Mapping of pipe names to lengths
        """
        pipe_lengths = {}
        try:
            for pipe_name in self.pipe_names:
