...
Implements demand adjustment, pressure difference calculation, and sensitivity matrix generation
"""
...
class SensitivityAnalyzer:
    ...
    def __init__(self, epanet_handler: EPANETHandler):
        ...
        self.normal_flow = None  # Store normal flow data
    ...
    def calculate_normal_pressure(self, duration_hours: int = 24, 
        ...
        """
        Calculate pressure data under normal conditions
        ...
        try:
            ...
            logger.info(f"Normal pressure data calculated: {self.normal_pressure.shape}")
            if not self.normal_flow.empty:
                logger.info(f"Normal flow data calculated: {self.normal_flow.shape}")
            return True
    ...
    def calculate_sensitivity_matrix(self, demand_ratio: float = 0.2,
        ...
        try:
            ...
            # sensitivity_matrix[i, j, t] represents the pressure sensitivity at node j at time t when node i is anomalous
            ...
            logger.info(f"Computing sensitivity matrix: {n_nodes} nodes, matrix shape: {self.sensitivity_matrix.shape}")
            
            # Iterate over each node, simulating its anomalous condition
            for i, anomaly_node in enumerate(tqdm(node_names, desc="Computing node sensitivity")):
                # Reset network
                self.epanet_handler.reset_network()
                
                # Modify current node's demand
                ...
                if anomaly_pressure.empty:
                    logger.warning(f"Anomalous pressure data is empty for node {anomaly_node}, skipping")
                    continue
                ...
                if anomaly_pressure.shape != self.normal_pressure.shape:
                    logger.warning(f"Anomalous pressure data shape mismatch for node {anomaly_node}: "
                                 f"normal {self.normal_pressure.shape} vs anomalous {anomaly_pressure.shape}, skipping")
                    continue
                ...
                for j, sensor_node in enumerate(node_names):
                    if sensor_node in pressure_diff.columns:
                        ...
                        if len(pressure_values) == self.sensitivity_matrix.shape[2]:
                            ...
                            # Use zero-padding or truncation
            ...
            logger.error(f"Failed to compute sensitivity matrix: {e}")
            return False
    
    def normalize_sensitivity_matrix(self) -> bool:
        """
        Normalize sensitivity matrix (standardization + normalization, handling zero denominators)
        ...
        try:
            if self.sensitivity_matrix is None:
                logger.error("Sensitivity matrix has not been computed")
                return False
            ...
            for i in range(n_nodes):
                for j in range(n_nodes):
                    ...
                    # Standardization: handle zero standard deviation
                    if std_val < epsilon:
                        # If standard deviation is 0, set to mean (if mean is also 0, keep as 0)
            ...
            for i in range(n_nodes):
                for j in range(n_nodes):
                    ...
                    if abs(max_val - min_val) < epsilon:
                        # If range is 0, set to 0.5 (midpoint)
                        self.sensitivity_matrix[i, j, :] = 0.5
                    else:
                        self.sensitivity_matrix[i, j, :] = (data - min_val) / (max_val - min_val + epsilon)

            logger.info("Sensitivity matrix standardization and normalization complete")
            return True
    ...
    def calculate_node_weights(self) -> np.ndarray:
        """
        Calculate node weight vector (temporal average of pressure sensitivity)
        ...
        try:
            ...
            # Calculate temporal average as node weights
            self.node_weights = np.mean(self.sensitivity_matrix, axis=2)
            
            logger.info(f"Node weights computed: {self.node_weights.shape}")
            return self.node_weights
            
        except Exception as e:
            logger.error(f"Failed to compute node weights: {e}")
            return np.array([])
